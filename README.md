...
1. SHAP 분석 결과 **Family_history_diabetes** 가 영향력 1위로 모델이 당뇨병 발생 여부를 판단할 때 환자의 현재 상태보다 유전적 배경을 가장 중요한 기초정보로 활용하고 있음
...
## ✔️8. Conclusion
- 통계적 유의성 및 임상적 위험요인 확정
 > BMI, 중성지방, 혈압은 통계적으로 매우 유효한 (p-value<0.05) 위험 요인이 증명되었으며 HDL 콜레스테롤 오즈비가 1.0미만인 변수들을 통해 '보호 효과'를 가짐을 임상적으로 확인함
- 통계와 머신러닝의 융합을 통한 데이터 기반 전략
 > BMI,혈압 모델기여도가 높은 환경적 요인을 결합하여 가족력이 있는 고위험군일수록 신체 활동량 관리가 당뇨병 발병 억제에 결정적인 기여를 한다는 데이터 기반의 명확한 결론을 도출


# 보고서
- 프로젝트 상세 보고서는 PDF 슬라이드 자료를 참고하여 주세요
- 보고서 : [당뇨병 예측 모델링 : 통계분석 및 머신러닝 접근](report/)
